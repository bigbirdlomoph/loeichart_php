...
?>
<?php
$strExcelFileName="60up_comservice_indiv_export_excel.xls";
header("Content-Type: application/x-msexcel; name=\"$strExcelFileName\"");
header("Content-Disposition: inline; filename=\"$strExcelFileName\"");
header("Pragma:no-cache");

 $db=new Database(DB_HOST_TK,DB_USER_TK,DB_PASS_TK,DB_NAME_TK);
 $db->Query("select * from z42_60up_comservice_t where HOSPCODE=$hospcode and VHID=$vhid and date_serv is not null ORDER BY DATE_SERV,CID");
 $num=$db->Numrow();
?>
