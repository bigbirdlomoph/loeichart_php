 ...
 //select name report
  $dbrpt->Query("SELECT * FROM z42_sys_report WHERE id='$id' ORDER BY cat_id");
    while($arrrpt=$dbrpt->Fetch_array()){
        ...
        <a href="../exportxls/60up_functional_indiv_export_excel.php?id=<?php echo $id;?>&hospcode=<?php echo $hospcode;?>&vhid=<?php echo $vhid;?>" class="btn btn-default btn-sm " role="button">Excel</a>
        </h4>
		<?php
        }
...
<?php
 ...
 $i=1; 
 $dbrpt->Query("select * from z42_60up_functional_t where HOSPCODE=$hospcode and VHID=$vhid and DATE_SERV is not null ");
...
?>    
	<tr>
        ...
        <td>
        <?php echo  $arr_rs['FUNCTIONAL_TEST']; ?>
		</td>
		<td>
        <?php echo  $arr_rs['TESTRESULT']; ?>
		</td>
        <td>
        <?php echo  $arr_rs['DEPENDENT']; ?>
		</td>
        <td>
        <?php echo  $arr_rs['BED']; ?>  
		</td>
