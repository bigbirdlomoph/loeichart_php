...
<title>LOEI CHART | REPORT DATA</title>
  ...
  <link href="../../bower_components/datatables/media/css/jquery.dataTables.min.css">
    <link href="../../bower_components/datatables/media/css/jquery.dataTables.css">
    <link href="../../bower_components/datatables/media/css/dataTables.tableTools.css">
    <!-- Custom Fonts -->
    <link href="../../bower_components/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">
...
if($age==9){
	if($fyear==2558){
		...
		$table=substr($table=$_POST['table'],0,16).substr($fyear,2,2).substr($table=$_POST['table'],-4,4);
	}
}else{
	if($fyear==2558){
     	$table=$_POST['table'];
	}else{
		$table=substr($table=$_POST['table'],0,18).substr($fyear,2,2).substr($table=$_POST['table'],-4,4);
	}
 ...
 //select name report
  $dbrpt->Query("SELECT * FROM z42_sys_report WHERE id='$id' ORDER BY cat_id");
    while($arrrpt=$dbrpt->Fetch_array()){
        ...
        <a href="../exportxls/child_develop_amp_export_excel.php?id=<?php echo $id?>&fyear=<?php echo $fyear;?>&age=<?php echo $age?>&table=<?php echo $table?>"class="btn btn-default btn-sm " role="button">
   ส่งออก Excel
   </a>
        </h4>
		<?php
...
<!---->
 <div class="panel panel-info">
  <div class="panel-heading"><?php echo $arrrpt['report_name']; ?>
...
<thead >
          <tr class="bg-primary">
          <th rowspan="3" class="text-center">อำเภอ</th>
...
?>    
	<tr>
		...
		<td class="bg-info">
         <a href="javascript:popup('child_develop_pcu.php?id=<?php echo $id;?>&ampcode=<?php echo $ampcode;?>&table=<?php echo $table?>&age=<?php echo $age;?>&fyear=<?php echo $fyear;?>','',960,500)"><?php echo $arr_rs['AMP_NAME']; ?></a> 
		</td>
        <td align="center">
