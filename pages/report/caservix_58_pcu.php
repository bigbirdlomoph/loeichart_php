...
จำนวนผู้ป่วยมะเร็งเต้านม แยกรายอำเภอ ปีงบประมาณ 2558 (ข้อมูล ณ เดือนมิถุนายน 2558)
...
<!--  <tr>
 ...
 <tr>
    ...
    <th><div align="center">จำนวนผู้ป่วยมะเร็ง</div></th>
...
<?php
 ...
 $dbrpt->Query("
        SELECT 
          42loei.off_name AS HNAME, 42loei.subdistid AS AREACODE,
          42loei.off_id AS HCODE,
          SUM(CASE WHEN CID<>'' THEN 1 ELSE 0 END)AS CC
          FROM z42_tc_cacervix c 
          INNER JOIN z42_co_office_loei AS 42loei on 42loei.off_id = c.HOSPCODE 
          AND 42loei.distid='$ampcode' 
          GROUP BY c.HOSPCODE 
       UNION ALL
        SELECT 
          'รวม' AS HNAME, 42loei.subdistid AS AREACODE,
          'รวม' AS HCODE,
          SUM(CASE WHEN CID<>'' THEN 1 ELSE 0 END)AS CC
          FROM z42_tc_cacervix c 
          INNER JOIN z42_co_office_loei AS 42loei on 42loei.off_id = c.HOSPCODE  
...
?>    
	<tr>
		<td>
         <a href="javascript:popup('caservix_58_cid.php?hcode=<?=$arr_rs['HCODE'];?>&villid=<?=$arr_rs['AREACODE'];?>&b_year=<?php echo $b_year;?>&ystr=<?php echo $ystr;?>&yend=<?php echo $yend;?>','',960,500)"><?php echo $arr_rs['HNAME']; ?></a></td>
