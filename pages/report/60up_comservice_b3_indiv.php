 ...
 //select name report
  $dbrpt->Query("SELECT * FROM z42_sys_report WHERE id='$id' ORDER BY cat_id");
    while($arrrpt=$dbrpt->Fetch_array()){
        ...
        <a href="../exportxls/60up_comservice_b3_indiv_export_excel.php?id=<?php echo $id;?>&hospcode=<?php echo $hospcode;?>&vhid=<?php echo $vhid;?>" class="btn btn-default btn-sm " role="button">Excel</a>
        </h4>
		<?php
        }
...
<?php
 ...
 $i=1; 
 $dbrpt->Query("select * from z42_60up_comservice_t cs  LEFT JOIN z42_60up_functional_t f ON cs.CID=f.CID where f.BED='3' and cs.DATE_SERV is not null and cs.HOSPCODE=$hospcode and cs.VHID=$vhid  ORDER BY cs.CID,cs.DATE_SERV");
