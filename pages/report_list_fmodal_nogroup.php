                  ...
                  <ul class="list-group"> 
					<?php  
					 $dbreport=new Database(DB_HOST_TK,DB_USER_TK,DB_PASS_TK,DB_NAME_TK);
	 				 $dbreport->Query("SELECT * FROM z42_sys_report WHERE cat_id='$catid' and active='1' ORDER BY id_sort");
	   					       while($arrreport=$dbreport->Fetch_array()){
								  
	 				
					?>
                      <li class="list-group-item">
               <a href="<?php echo $arrreport['source_file']?><?php echo $arrreport['para_meter'] ?>id=<?php echo $arrreport['id'];?>&table=<?php echo $arrreport['source_table_s'];?>" target="new"> <?php echo $arrreport['report_name'];?>&nbsp;<i class="fa fa-check"></i></a>
